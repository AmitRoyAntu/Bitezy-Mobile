...
const AdminDashboardScreen = ({ navigation }) => {
  ...
  const canteenRevenueList = useMemo(() => {
    return providers.map((prov) => {
      ...
      const share = totalRevenue > 0 ? ((provRev / totalRevenue) * 100).toFixed(1) : 0;
      return {
        provider: prov,
        name: prov.name,
          ...
          <View style={styles.modalCard}>
            <View style={styles.modalHeader}>
              <View>
                ...
                <Text style={styles.modalSubtitle}>Total Gross Volume per Canteen</Text>
              </View>
              <TouchableOpacity
            ...
            <View style={styles.revenueSummaryCard}>
              <View style={styles.revSummaryItem}>
                ...
                <Text style={styles.revSummaryValue}>৳{totalRevenue.toLocaleString()}</Text>
              </View>
              <View style={styles.summaryDivider} />
              <View style={styles.revSummaryItem}>
                <Text style={styles.revSummaryLabel}>Food Sales</Text>
                <Text style={[styles.revSummaryValue, { color: colors.textDark }]}>
            ...
            {/* List of Canteens */}
            <Text style={styles.modalSectionLabel}>Revenue by Canteen (Ranked):</Text>
            <ScrollView style={{ maxHeight: 320 }} showsVerticalScrollIndicator={false}>
              {canteenRevenueList.map((item, idx) => (
                <View key={item.provider._id || idx} style={styles.breakdownItem}>
                  ...
                  <View style={styles.breakdownStats}>
                    <Text style={styles.breakdownAmount}>৳{item.revenue.toLocaleString()}</Text>
                    <Text style={styles.breakdownOrders}>{item.ordersCount} orders ({item.share}%)</Text>
                  </View>
                </View>
              ))}
            </ScrollView>
          ...
          <View style={styles.modalCard}>
            <View style={styles.modalHeader}>
              <View>
                ...
                <Text style={styles.modalSubtitle}>{orders.length} total orders recorded</Text>
              </View>
              <TouchableOpacity
                style={styles.closeBtn}
                onPress={() => setActiveModal(null)}
          ...
          <View style={styles.modalCard}>
            <View style={styles.modalHeader}>
              <View>
                ...
                <Text style={styles.modalSubtitle}>{buyers.length} registered campus buyers</Text>
              </View>
              <TouchableOpacity
                style={styles.closeBtn}
                onPress={() => setActiveModal(null)}
            ...
            <View style={styles.demographicGrid}>
              ...
              <View style={[styles.demoCard, { borderColor: colors.dangerBorder }]}>
                <Ionicons name="ban" size={24} color={colors.danger} />
                <Text style={[styles.demoValue, { color: colors.danger }]}>{userTypeCounts.Blocked}</Text>
            ...
            >
              <Text style={styles.modalActionBtnText}>Manage Users in Users Tab</Text>
              <Ionicons name="arrow-forward" size={16} color={colors.white} />
            </TouchableOpacity>
          </View>
        </View>
          ...
          <View style={styles.modalCard}>
            <View style={styles.modalHeader}>
              <View>
                ...
                <Text style={styles.modalSubtitle}>{providers.length} registered food providers</Text>
              </View>
              <TouchableOpacity
                style={styles.closeBtn}
                onPress={() => setActiveModal(null)}
              ...
              {providers.map((p) => (
                <View key={p._id || p.id} style={styles.sellerModalItem}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={styles.sellerModalLoc}>{p.location || 'CUET Campus'}</Text>
                  </View>
                  <View
                    style={[
                      styles.statusIndicator,
                      { backgroundColor: p.isBlocked ? colors.dangerLight : colors.successLight },
                    ]}
                  >
                    <Text
                      style={[
            ...
            >
              <Text style={styles.modalActionBtnText}>Manage Sellers in Sellers Tab</Text>
              <Ionicons name="arrow-forward" size={16} color={colors.white} />
            </TouchableOpacity>
          </View>
        </View>
          ...
          <View style={styles.modalCard}>
            ...
            {selectedOrder && (
              <ScrollView showsVerticalScrollIndicator={false}>
                ...
                <Text style={[styles.modalSectionLabel, { marginTop: 12 }]}>Items Ordered:</Text>
                <View style={styles.orderItemsBox}>
                  {(selectedOrder.items || []).map((item, i) => (
                    <View key={i} style={styles.itemLine}>
                      ...
                      <Text style={styles.itemLinePrice}>৳{(item.price || 0) * (item.qty || 1)}</Text>
                    </View>
                  ))}
                </View>
